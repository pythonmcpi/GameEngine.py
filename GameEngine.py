...
class Screen:
    def __init__(self, size, pos, title):
        ...
        self.root.geometry(
            str(self.windowSizeX)
            + "x"
            + str(self.windowSizeY)
            + "+"
            + str(self.windowPosX)
            + "+"
            + str(self.windowPosY)
        ...
        self.canvas = tkinter.Canvas(
            self.root, bd=5, height=self.windowSizeY, width=self.windowSizeX
        )
        self.canvas.pack()
    ...
    def fillbg(self, color):
        ...
        self.canvas.create_rectangle(
            0, 0, self.windowSizeX, self.windowSizeY, fill=colorHex
        )
    ...
    def drawline(
        self, pos1, pos2, color, width=1
    ):  # https://kite.com/python/docs/tkinter.Canvas.create_line
        colorHex = self.RGB2Hex(color)
        pos1x = pos1[0]
        pos1y = pos1[1]
        pos2x = pos2[0]
    ...
    def write(
        self, text, pos, color, font=("Arial", 12, False, False, False)
    ...
    def drawcircle(
        self, x, y, r, color
    ):  # https://stackoverflow.com/questions/17985216/draw-circle-in-tkinter-python by Edgar Runnman with slight modifications to fit the class
        colorHex = self.RGB2Hex(color)
        x0 = (
            x - r
        )  # Convert center coordinates to the bounding box coordinates (tkinter is hard to use lol :P)
        y0 = y - r
        x1 = x + r
        y1 = y + r
